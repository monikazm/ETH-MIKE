...
for i = 1:length(C(:,5))
    if C(i,4) == 0 % right hand impaired 
        MEP_Lat_Imp(n,1) = C(i,1); 
        MEP_Lat_Imp(n,2) = C(i,2);
        MEP_Lat_Imp(n,3) = C(i,6); 
        MEP_Lat_NonImp(n,1) = C(i,1); 
        MEP_Lat_NonImp(n,2) = C(i,2);
        MEP_Lat_NonImp(n,3) = C(i,5); 
        n = n+1; 
    elseif C(i,4) == 1 % left hand impaired 
        MEP_Lat_Imp(n,1) = C(i,1); 
        MEP_Lat_Imp(n,2) = C(i,2);
        MEP_Lat_Imp(n,3) = C(i,5); 
        MEP_Lat_NonImp(n,1) = C(i,1); 
        MEP_Lat_NonImp(n,2) = C(i,2);
        MEP_Lat_NonImp(n,3) = C(i,6); 
        n = n+1; 
    end
end
...
MEP_Lat_Imp_S1 = MEP_Lat_Imp(find(MEP_Lat_Imp(:,2)==1),:); 
MEP_Lat_Imp_S2 = MEP_Lat_Imp(find(MEP_Lat_Imp(:,2)==2),:);

MEP_Lat_NonImp_S1 = MEP_Lat_NonImp(find(MEP_Lat_NonImp(:,2)==1),:); 
MEP_Lat_NonImp_S2 = MEP_Lat_NonImp(find(MEP_Lat_NonImp(:,2)==2),:);
