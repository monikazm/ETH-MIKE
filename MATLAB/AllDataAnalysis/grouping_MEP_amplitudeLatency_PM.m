...
for i=1:length(A)
    if (A(i,6) == 35 && A(i,2) == 0) 
        ...
        latamp.T1.abs(n,5) = A(i,18); % PM T1
        latamp.T1.abs(n,6) = A(i,19); % PM T3
        latamp.T1.abs(n,7) = A(i,18)-A(i,19); % PM Delta
        n = n+1; 
    elseif A(i,6)-A(i,8) > 1.2 || A(i,2) < 0.5*A(i,4) 
        ...
        latamp.T1.imp(m,5) = A(i,18); % PM T1
        latamp.T1.imp(m,6) = A(i,19); % PM T3
        latamp.T1.imp(m,7) = A(i,18)-A(i,19); % PM Delta
        m = m+1; 
    elseif A(i,6)-A(i,8) <= 1.2 && A(i,2) >= 0.5*A(i,4)
        ...
        latamp.T1.norm(k,5) = A(i,18); % PM T1
        latamp.T1.norm(k,6) = A(i,19); % PM T3
        latamp.T1.norm(k,7) = A(i,18)-A(i,19); % PM Delta
        k = k+1; 
    end
end
...
for i=1:length(latamp.T1.abs(:,7))
    if latamp.T1.abs(i,1) == 5 || latamp.T1.abs(i,1) == 8 || latamp.T1.abs(i,1) == 13  || latamp.T1.abs(i,1) == 14 || latamp.T1.abs(i,1) == 19  || latamp.T1.abs(i,1) == 21 || latamp.T1.abs(i,1) == 24  || latamp.T1.abs(i,1) == 30 || latamp.T1.abs(i,1) == 34
...
for i=1:length(latamp.T1.imp(:,7))
    if latamp.T1.imp(i,1) == 5 || latamp.T1.imp(i,1) == 8 || latamp.T1.imp(i,1) == 13  || latamp.T1.imp(i,1) == 14 || latamp.T1.imp(i,1) == 19  || latamp.T1.imp(i,1) == 21 || latamp.T1.imp(i,1) == 24  || latamp.T1.imp(i,1) == 30 || latamp.T1.imp(i,1) == 34
...
for i=1:length(latamp.T1.norm(:,7))
    if latamp.T1.norm(i,1) == 5 || latamp.T1.norm(i,1) == 8 || latamp.T1.norm(i,1) == 13  || latamp.T1.norm(i,1) == 14 || latamp.T1.norm(i,1) == 19  || latamp.T1.norm(i,1) == 21 || latamp.T1.norm(i,1) == 24  || latamp.T1.norm(i,1) == 30 || latamp.T1.norm(i,1) == 34
...
print('plots/BoxPlots/211116_MEPamplatT1_PM_Delta','-dpng')
