...
for i=1:length(A)
    if (A(i,6) == 35 && A(i,2) == 0) 
        ...
        latamp.T1.abs(n,5) = A(i,15)-A(i,14); % Delta BBT
        n = n+1; 
    elseif A(i,6)-A(i,8) > 1.2 || A(i,2) < 0.5*A(i,4) % medium
        ...
        latamp.T1.imp(m,5) = A(i,15)-A(i,14); % Delta BBT
        m = m+1; 
    elseif A(i,6)-A(i,8) <= 1.2 && A(i,2) >= 0.5*A(i,4) % normal 
        ...
        latamp.T1.norm(k,5) = A(i,15)-A(i,14); % Delta BBT
        k = k+1; 
    end
end
...
ylim([-2 27])
set(gca,'FontSize',12)
title('Delta BBT vs SSEP groups') 
%print('plots/Paper/20220522_Figure3A','-dpng')
