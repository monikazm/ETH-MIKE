...
%% boxplot - baseline proprioception in subjects that changed in motor only vs in both vs in none 

g1 = repmat({'Motor (N=17)'},length(motor.S1(:,1)),1);
g2 = repmat({'Both (N=8)'},length(both.S1(:,1)),1);
g3 = repmat({'None (N=15)'},length(neither.S1(:,1)),1);
g = [g1;g2;g3]; 

figure; 
boxplot([motor.S1(:,7); both.S1(:,7); neither.S1(:,7)],g) 
...
for i=1:length(motor.S1(:,7))
    scatter(1,motor.S1(i,7),'filled','k'); 
end
for i=1:length(both.S1(:,7))
    scatter(2,both.S1(i,7),'filled','k');
end
for i=1:length(neither.S1(:,7))
    scatter(3,neither.S1(i,7),'filled','k');
end
xlabel('Groups') 
ylabel('Position Matching Absolute Error (deg) @ T1') 
print('plots/BoxPlots/211126_groups_PMT1','-dpng')

%% kruskal wallis between groups

p_groups1 = kruskalwallis([motor.S1(:,7); both.S1(:,7)],[g1;g2]); 
p_groups2 = kruskalwallis([motor.S1(:,7); neither.S1(:,7)],[g1;g3]); 
gr = [g3;g2]; 
p_groups3 = kruskalwallis([motor.S1(:,7); [neither.S1(:,7) both.S1(:,7)]],[g1;gr]); 
