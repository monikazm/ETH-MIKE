...
g1 = repmat({'Change both (N=8)'},length(both.S1(:,1)),1);
g2 = repmat({'Change one (N=19)'},length([motor.S1(:,1); sensory.S1(:,1)]),1);
g3 = repmat({'No change both (N=15)'},length(neither.S1(:,1)),1);
g = [g1;g2;g3]; 

figure; 
boxplot([both.S3(:,5); [motor.S3(:,5); sensory.S3(:,5)]; neither.S3(:,5)],g) 
...
hold on 
for i=1:length(both.S3(:,5))
    scatter(1,motor.S3(i,5),'filled','k'); 
end
temp = [motor.S3(:,5); sensory.S3(:,5)]; 
for i=1:length(temp)
    scatter(2,temp(i),'filled','k');
end
for i=1:length(neither.S3(:,5))
    scatter(3,neither.S3(i,5),'filled','k'); 
end
ylim([-5 65]) 
xlabel('Groups') 
ylabel('BBT @ T3') 
print('plots/BoxPlots/211202_ChangeGroups_BBTT3','-dpng')

figure; 
boxplot([both.S1(:,5); [motor.S1(:,5); sensory.S1(:,5)]; neither.S1(:,5)],g) 
b = findobj(gca,'tag','Median');
set(b,{'linew'},{2})
colors = {[0.85, 0.85, 0.85], [0.85, 0.85, 0.85], [0.85, 0.85, 0.85]};  
h = findobj(gca,'Tag','Box');
...
hold on 
for i=1:length(both.S1(:,5))
    scatter(1,motor.S1(i,5),'filled','k'); 
end
temp = [motor.S1(:,5); sensory.S1(:,5)]; 
for i=1:length(temp)
    scatter(2,temp(i),'filled','k');
end
for i=1:length(neither.S1(:,5))
    scatter(3,neither.S1(i,5),'filled','k'); 
end
xlabel('Groups') 
ylabel('BBT @ T1') 
ylim([-5 65]) 
print('plots/BoxPlots/211202_ChangeGroups_BBTT1','-dpng')


%statistical difference  
p_bbt = kruskalwallis([both.S1(:,12); temp],[g1;g2]); 
