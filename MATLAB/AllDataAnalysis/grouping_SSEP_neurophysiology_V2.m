...
for i=1:length(A)
    if A(i,7) == 35
        ...
        lat.T3.abs(n,4) = A(i,11); % PM T3
        lat.T3.abs(n,5) = A(i,13); % kUDT T3
        n = n+1; 
    elseif A(i,7)-A(i,9) > 1.1
        lat.T3.imp(m,1) = A(i,1); 
        lat.T3.imp(m,2) = A(i,7); 
        lat.T3.imp(m,3) = A(i,9);
        lat.T3.imp(m,4) = A(i,7)-A(i,9);
        lat.T3.imp(m,5) = A(i,11); % PM T3
        lat.T3.imp(m,6) = A(i,13); % kUDT T3
        m = m+1; 
    elseif A(i,7)-A(i,9) <= 1.1
...
% Kruskal Wallis
p_PM_T1_lat = kruskalwallis([lat.T1.abs(:,4); lat.T1.norm(:,5)],[g1;g3]); 

%% SSEP latency groups - T3


% PM
g1 = repmat({'Absent'},length(lat.T3.abs(:,4)),1);
g2 = repmat({'Impaired'},length(lat.T3.imp(:,5)),1);
g3 = repmat({'Normal'},length(lat.T3.norm(:,5)),1);
g = [g1;g2;g3]; 
figure; 
boxplot([lat.T3.abs(:,4); lat.T3.imp(:,5); lat.T3.norm(:,5)],g) 
title('T3')
xlabel('SSEP latency @ T3') 
ylabel('Position Matching Absolute Error (deg) @ T3') 
print('plots/BoxPlots/211101_SSEPlat_PM_T3','-dpng')

% calculations
lat.T3.norm_PMmedian = nanmedian(lat.T3.norm(:,5)); 
lat.T3.norm_PMiqr = iqr(lat.T3.norm(:,5)); 
lat.T3.imp_PMmedian = nanmedian(lat.T3.imp(:,5)); 
lat.T3.imp_PMiqr = iqr(lat.T3.imp(:,5)); 
lat.T3.abs_PMmedian = nanmedian(lat.T3.abs(:,4)); 
lat.T3.abs_PMiqr = iqr(lat.T3.abs(:,4)); 
