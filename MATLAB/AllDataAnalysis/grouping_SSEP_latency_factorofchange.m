...
%% plot SSEP latency groups vs PM change

% PM
g1 = repmat({'Absent'},length(lat.T1.abs(:,5)),1);
g2 = repmat({'Impaired'},length(lat.T1.imp(:,5)),1);
g3 = repmat({'Normal'},length(lat.T1.norm(:,5)),1);
g = [g1;g2;g3]; 
figure; 
boxplot([lat.T1.abs(:,7); lat.T1.imp(:,7); lat.T1.norm(:,7)],g) 
...
hold on 
for i=1:length(lat.T1.abs(:,7))
    scatter(1,lat.T1.abs(i,7),'filled','k'); 
end
hold on
for i=1:length(lat.T1.imp(:,7))
    scatter(2,lat.T1.imp(i,7),'filled','k'); 
end
hold on
for i=1:length(lat.T1.norm(:,7))
    scatter(3,lat.T1.norm(i,7),'filled','k'); 
end
xlabel('SSEP latency @ T1') 
ylabel('Delta Position Matching Absolute Error (deg)') 
print('plots/BoxPlots/211115_SSEPlat_PM_Delta_v1','-dpng')
...
lat.T1.absimp = [lat.T1.abs;lat.T1.imp];  

% PM
g1 = repmat({'Impaired'},length(lat.T1.absimp(:,7)),1);
g2 = repmat({'Normal'},length(lat.T1.norm(:,7)),1);
g = [g1;g2]; 
figure; 
boxplot([lat.T1.absimp(:,7); lat.T1.norm(:,7)],g) 
b = findobj(gca,'tag','Median');
set(b,{'linew'},{2})
...
hold on 
for i=1:length(lat.T1.absimp(:,7))
    if lat.T1.absimp(i,7) >= 9.12
       scatter(1,lat.T1.absimp(i,7),'filled','g'); 
       hold on 
       labelpoints(1,lat.T1.absimp(i,7), string(lat.T1.absimp(i,1))); 
    elseif lat.T1.absimp(i,6) <= 10.63 && lat.T1.absimp(i,5) > 10.63
       scatter(1,lat.T1.absimp(i,7),'filled','b'); 
       hold on 
       labelpoints(1,lat.T1.absimp(i,7), string(lat.T1.absimp(i,1))); 
    else
       scatter(1,lat.T1.absimp(i,7),'filled','k');
       hold on 
       labelpoints(1,lat.T1.absimp(i,7), string(lat.T1.absimp(i,1))); 
    end
end
hold on
for i=1:length(lat.T1.norm(:,7))
    if lat.T1.norm(i,7) >= 9.12
       scatter(2,lat.T1.norm(i,7),'filled','g'); 
       hold on 
       labelpoints(2,lat.T1.norm(i,7), string(lat.T1.norm(i,1))); 
    elseif lat.T1.norm(i,6) <= 10.63 && lat.T1.norm(i,5) > 10.63
       scatter(2,lat.T1.norm(i,7),'filled','b'); 
       hold on 
       labelpoints(2,lat.T1.norm(i,7), string(lat.T1.norm(i,1))); 
    else
       scatter(2,lat.T1.norm(i,7),'filled','k'); 
       hold on 
       labelpoints(2,lat.T1.norm(i,7), string(lat.T1.norm(i,1))); 
    end
end
xlabel('SSEP latency @ T1') 
ylabel('Delta Position Matching Absolute Error (deg)') 
print('plots/BoxPlots/211115_SSEPlat_PM_Delta_v2','-dpng')

% is the difference significant between the groups? 
% Kruskal Wallis
p_PMDelta_SSEPT1 = kruskalwallis([lat.T1.absimp(:,7); lat.T1.norm(:,7)],[g1;g2]); 
...
xlabel('SSEP latency @ T1') 
ylabel('Delta Position Matching Absolute Error (deg)') 
print('plots/BoxPlots/211115_SSEPlat_PM_Delta_v3','-dpng')


%% initial impairment groups

% PM
g1 = repmat({'Impaired'},length(lat.T1.absimp(:,5)),1);
g2 = repmat({'Normal'},length(lat.T1.norm(:,5)),1);
g = [g1;g2]; 
figure; 
boxplot([lat.T1.absimp(:,5); lat.T1.norm(:,5)],g) 
b = findobj(gca,'tag','Median');
set(b,{'linew'},{2})
colors = {[0.85, 0.85, 0.85], [0.85, 0.85, 0.85]};  
h = findobj(gca,'Tag','Box');
...
hold on 
for i=1:length(lat.T1.absimp(:,5))
    if lat.T1.absimp(i,7) >= 9.12
       scatter(1,lat.T1.absimp(i,5),'filled','k'); 
    elseif lat.T1.absimp(i,6) <= 10.63 && lat.T1.absimp(i,5) > 10.63
       scatter(1,lat.T1.absimp(i,5),'filled','k'); 
    else
       scatter(1,lat.T1.absimp(i,5),'filled','k');
...
hold on
for i=1:length(lat.T1.norm(:,5))
    if lat.T1.norm(i,7) >= 9.12
       scatter(2,lat.T1.norm(i,5),'filled','k'); 
    elseif lat.T1.norm(i,6) <= 10.63 && lat.T1.norm(i,5) > 10.63
       scatter(2,lat.T1.norm(i,5),'filled','k'); 
    else
       scatter(2,lat.T1.norm(i,5),'filled','k'); 
...
print('plots/BoxPlots/211115_SSEPlat_PM_T1_v2','-dpng')

% is the difference significant between the groups? 
% Kruskal Wallis
p_PMT1_SSEPT1 = kruskalwallis([lat.T1.absimp(:,5); lat.T1.norm(:,5)],[g1;g2]); 
