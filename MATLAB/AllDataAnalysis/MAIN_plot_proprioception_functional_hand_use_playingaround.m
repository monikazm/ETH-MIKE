...
A = table2array(T(:,[3 5 47 41 48 44 122 114 92 160 50])); 
...
figure; 
for i = 1:length(S3(:,1))
    if S1(i,5)>=30 %&& S3(i,5)-S1(i,5)<15 && S3(i,5)-S1(i,5)>-5
    tmp(n,1)=S3(i,5)-S1(i,5); 
    tmp(n,2)=S1(i,7); 
    scatter(tmp(n,2),tmp(n,1),'filled','k');
    n = n +1; 
...
[rho.DBBTPM1_2,pval.DBBTPM1_2] = corr(tmp(:,1),tmp(:,2), 'Type', 'Spearman');
...
m = 1; 
for i = 1:length(S1(:,1))
    if S3(i,5)-S1(i,5) >= 5.5 && S1(i,5)>=3
        changeBBT.S1(n,:) = S1(i,:); 
        changeBBT.S3(n,:) = S3(i,:); 
        n = n+1; 
    elseif S3(i,5)-S1(i,5) < 5.5 && S1(i,5)>=3
        nochangeBBT.S1(m,:) = S1(i,:); 
        nochangeBBT.S3(m,:) = S3(i,:); 
        m = m+1; 
    end
end

% delta BBT
g1 = repmat({'Change (N=18)'},length(changeBBT.S1(:,1)),1);
g2 = repmat({'No change (N=8)'},length(nochangeBBT.S1(:,1)),1);
g = [g1;g2]; 
figure; 
boxplot([changeBBT.S1(:,7); nochangeBBT.S1(:,7)],g) 
b = findobj(gca,'tag','Median');
set(b,{'linew'},{2})
colors = {[0.85, 0.85, 0.85], [0.85, 0.85, 0.85]};  
h = findobj(gca,'Tag','Box');
...
hold on 
for i=1:length(changeBBT.S1(:,7))
    scatter(1,changeBBT.S1(i,7),'filled','k'); 
end
for i=1:length(nochangeBBT.S1(:,7))
    scatter(2,nochangeBBT.S1(i,7),'filled','k');
end
hold on
...
% sensory change groups 
p_groupsBBT_PM = kruskalwallis([changeBBT.S1(:,7); nochangeBBT.S1(:,7)],g); 
