...
% lognitudinal plotting, grouping, Target Following Smoothness MAPR (Slow)
...
task = 2 + 5; 
    ...
    for i = 1:length(S1(:,1))
        if S1(i,task) <= 0.5
            G3(n,1) = S1(i,1); 
            n = n+1; 
        elseif S1(i,task) > 0.5 && S1(i,task) <= 0.8
            G2(m,1) = S1(i,1); 
            m = m+1; 
        elseif S1(i,task) > 0.8
            G1(k,1) = S1(i,1); 
            k = k+1; 
        end
    end
...
G1_mean(1,4) = ((G1_mean(1,3)*10 - G1_mean(1,1)*10)/G1_mean(1,1)*10);
...
G2_mean(1,4) = ((G2_mean(1,3)*10 - G2_mean(1,1)*10)/G2_mean(1,1)*10);
...
G3_mean(1,4) = ((G3_mean(1,3)*10 - G3_mean(1,1)*10)/G3_mean(1,1)*10);
...
ylabel('Target Following Smoothness MAPR') 
print('Plots/LongitudinalPlots/robotic/groups/210701_Sum_MAPRS_groups','-dpng')
